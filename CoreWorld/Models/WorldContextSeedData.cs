...
namespace CoreWorld.Models
{
    public class WorldContextSeedData
    {
        ...
        {
            if (await _userManager.FindByEmailAsync("astrip@worldtrip.com") == null)
            {
                var user = new WorldUser()
                {
                    UserName = "astrip",
                    Email = "astrip@worldtrip.com",
                    FirstTrip = DateTime.UtcNow
                };

                IdentityResult result = await _userManager.CreateAsync(user, "Cbus123$");
            }
